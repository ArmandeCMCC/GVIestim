...
#   run multiple predefined attributable-burden
#   specifications in one go (same logic as single-run script).

#   Useful for robustness sweeps; final manuscript values were generated from
#   explicitly tagged specs.
#
# Batch-run several Paris attributable-deaths scenario specifications and save
# each one with a unique filename.
#
# Specifications included:
#   1) wbgt_mean × imu_high_popw × mmt
#   2) t2m_mean × imu_veg_total_popw × 22
#   3) t2m_mean × gvi_mean × 22
#   4) lst_max × imu_high_popw × mmt
#
# Outputs for each spec:
#   - paris_attr_scenario_rowlevel_<stub>.csv
#   - paris_attr_scenario_by_arr_<stub>.csv
#   - paris_attr_scenario_summary_<stub>.csv
...
make_effective_block <- function(beta_h, beta_g, V_hh, V_hg, V_gh, V_gg, g_value) {
  list(
    ...
    vcov = V_hh + g_value * V_hg + V_gh * g_value + (g_value^2) * V_gg
  )
}
...
format_threshold_token <- function(threshold_mode) {
  ...
  if (!is.finite(thr_num)) {
    stop("threshold_mode must be 'mmt' or numeric-like.")
  }
...
run_one_spec <- function(selected_heat, selected_green_std, threshold_mode, stub = NULL) {
  selected_green_raw <- sub("_std_iqr$", "", selected_green_std)
  if (is.null(stub) || !nzchar(stub)) {
    stub <- paste(
      sanitize_token(selected_heat),
      sanitize_token(selected_green_raw),
      format_threshold_token(threshold_mode),
      sep = "_"
    )
  }

  out_stub <- if (nzchar(run_tag)) paste0(stub, run_tag) else stub
  ...
  cat("\n============================================================\n")
  cat("Running spec:", stub, "\n")
  cat("  heat   =", selected_heat, "\n")
  cat("  green  =", selected_green_std, "\n")
  cat("  thresh =", threshold_mode, "\n")
  cat("============================================================\n")
  
  if (!(selected_heat %in% names(dt))) {
    stop("Selected heat metric not found: ", selected_heat)
  }
  if (!(selected_green_std %in% names(dt))) {
    stop("Selected greenness metric not found: ", selected_green_std)
  }
  if (!(selected_green_raw %in% names(dt))) {
    stop("Selected raw greenness metric not found: ", selected_green_raw)
  }
  ...
  if (threshold_mode == "mmt") {
    threshold_value <- heat_mmt
  } else {
    threshold_value <- as.numeric(threshold_mode)
    if (!is.finite(threshold_value)) {
      stop("threshold_mode must be 'mmt' or a numeric-like string.")
    }
  }
...
for (i in seq_len(nrow(specs))) {
  results[[i]] <- run_one_spec(
    selected_heat = specs$selected_heat[i],
    selected_green_std = specs$selected_green_std[i],
    threshold_mode = specs$threshold_mode[i],
    stub = specs$stub[i]
  )
}

cat("\nFinished all scenario runs.\n")
cat("Now rerun summarize_paris_scenario_comparison.R\n")
